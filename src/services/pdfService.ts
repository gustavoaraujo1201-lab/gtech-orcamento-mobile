...
export function criarHtmlPdf(orcamento: Orcamento, empresa: Empresa): string {
  ...
  const infoEmpresaHtml = infoEmpresa.map((linha) => `<div class="muted">${escaparHtml(linha)}</div>`).join('');
  const emissao = new Date();
  const dataEmissao = dataBrasileira(emissao.toISOString());
  const horaEmissao = new Intl.DateTimeFormat('pt-BR', { hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(emissao);
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>
    body{font-family:Arial,sans-serif;color:#17212b;padding:36px;font-size:12px}.header{border-bottom:3px solid #123b5d;padding-bottom:16px;display:flex;justify-content:space-between}.brand{color:#123b5d;font-size:20px;font-weight:bold;letter-spacing:1px}.title{font-size:22px;font-weight:bold;margin:24px 0 4px}.muted{color:#627180}.client{background:#f4f7fa;padding:14px;margin:18px 0;line-height:1.7}table{width:100%;border-collapse:collapse;margin-top:16px}th{text-align:left;background:#123b5d;color:#fff;padding:9px}td{padding:9px;border-bottom:1px solid #dce3ea}.number{text-align:right}.totals{margin:20px 0 0 auto;width:260px}.totals div{display:flex;justify-content:space-between;padding:6px 0}.total{font-weight:bold;font-size:16px;border-top:2px solid #123b5d;color:#123b5d}.footer{position:fixed;bottom:20px;left:36px;right:36px;display:flex;align-items:center;color:#627180;font-size:10px;border-top:1px solid #dce3ea;padding-top:8px}.footer-col{flex:1 1 0}.footer-left{text-align:left}.footer-center{text-align:center}.footer-right{text-align:right}
  </style></head><body><div class="header"><div><div class="brand">${escaparHtml(empresa.nome || 'GTECH PRIME')}</div>${infoEmpresaHtml}</div><div class="number"><b>ORÇAMENTO</b><br/>${orcamento.numero}<br/>${dataBrasileira(orcamento.criadoEm)}</div></div>
  ...
  ${orcamento.observacoes ? `<div class="client"><b>Observações</b><br/>${escaparHtml(orcamento.observacoes).replace(/\n/g, '<br/>')}</div>` : ''}<div class="footer"><div class="footer-col footer-left">${escaparHtml(dataEmissao)}</div><div class="footer-col footer-center">Sistema desenvolvido por GTech Prime</div><div class="footer-col footer-right">${escaparHtml(horaEmissao)}</div></div></body></html>`;
}
